...
dtb_path="${outp_path}/images/sun8i-h3-orangepi-lite.dtb"

if [ ! -f "$uimg_path" ]; then
  echo "zImage not found, make sure Buildroot is done compiling."
  echo "Looked at path: $uimg_path"
  exit 1
fi
...
$mkimage -A arm -C none -T script -n 'Skiff PiLite' -d $boot_dir/boot.txt $boot_dir/boot.scr
sync
