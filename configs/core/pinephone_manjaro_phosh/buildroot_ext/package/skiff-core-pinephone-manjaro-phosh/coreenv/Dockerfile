...
# pacman --noconfirm -S archlinux-keyring archlinuxarm-keyring && \
...
RUN \
    echo "" > /etc/fstab && \
    pacman --noconfirm -Rc anx7688-firmware \
...
RUN systemctl set-default graphical.target && \
    systemctl mask tmp.mount getty@tty1.service plymouth-quit-wait.service && \
